...
class PointAnomalyDetector:
    def __init__(self, config_path: str):
        # Loading of anomaly rules from the config file
        with open(config_path, "r") as file:
            config = yaml.safe_load(file) or {}
        rules = (config.get("point_anomalies") or {})
        self.template_rules: List[str] = rules.get("template_rules") or [] # Regex-based template anomaly detection patterns
        self.attribute_fields: List[str] = rules.get("attribute_fields") or [] # Rules for numeric fields for attribute anomaly detection
        self.iqr_factor: float = rules.get("iqr_factor", 1.2) # Setting IQR outlier sensitivity

    # Extraction of text to match it against regex rules
    ...
    # Extraction of a numeric value from log field
    def safe_float(self, val: Any) -> Optional[float]:
        ...
        for suf in ["%", "ms", "s"]:
            if s.endswith(suf):
                s = s[: -len(suf)].strip()
        numeric_match = re.search(r"([0-9]+(?:\.[0-9]+)?)", s)
        if numeric_match:
    ...
    # Checking for flexible matching of numeric attribute names
    ...
    # Checking for log message against regex known patterns
    ...
    # Finding numeric deviations using IQR method
        ...
        try:
            ...
            for field in fields:
                ...
                for log in parsed_logs:
                    ...
                    if num is None:
                        # Extraction of numbers directly from message text if field not found
                ...
                for log in parsed_logs:
                    raw = self.find_field_value(log, field)
                    num = self.safe_float(self.find_field_value(log, field))
                    if num is None:
                        message = log.get("message", "")
                        numbers = re.findall(r"\d+(?:\.\d+)?", message)
                        if numbers:
    ...
    # Execution of both anomaly detection methods and severity assignment
